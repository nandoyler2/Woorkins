...
export async function getOrCreateUserProfile(user: { id: string; email?: string }): Promise<Profile[]> {
  ...
  console.log('[getOrCreateUserProfile] Freeing identifier if owned by deleted profile...');
  await supabase.rpc('free_identifier_if_owner_deleted', { p_identifier: username });
