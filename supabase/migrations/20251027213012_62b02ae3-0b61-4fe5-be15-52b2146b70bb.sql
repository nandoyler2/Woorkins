-- Corrigir funções e triggers unificados de identificadores em profiles

-- 1) Funções (recriar por segurança)
...
BEGIN
  ...
  IF v_old_identifier IS DISTINCT FROM v_new_identifier THEN
...
-- 2) Triggers (remover antigos e recriar unificados)
...
DROP TRIGGER IF EXISTS trg_register_profile_identifier ON public.profiles;
DROP TRIGGER IF EXISTS trg_update_profile_identifier ON public.profiles;
